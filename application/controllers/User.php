...
class User extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->lang->load('auth');
		//redirect(str_replace('http://', 'https://', $this->config->config['base_url']));
	}
...
public function login(){
	...
	if ($this->form_validation->run() === TRUE)
	{
		...
		$check = $this->Common_model->login($this->input->post('identity'),$this->input->post('password'),$remember);
		if($check){
			$user_id =($check[0]->id);
			$verify =$this->Common_model->select("2_veri",$user_id);
			...
			if(!empty($verify)){
				...
				// redirect('/');
					...
					$return_arr = array('userId'=>$user_id,'identity'=>$this->input->post('identity'),'password'=>$this->input->post('password'));
...
public function dashboard()
{
	...
	if($this->ion_auth->logged_in()){
		...
		$data_eth =$this->Common_model->calculate_rasied('eth');
		$data_btc =$this->Common_model->calculate_rasied('btc');
		$transc = file_get_contents('https://min-api.cryptocompare.com/data/price?fsym=BTC&tsyms=ETH,USD,EUR');
		$datacheck =(json_decode($transc));
		$btc =(($datacheck->ETH)*$data_btc[0]->Total);
		$eth = $data_eth[0]->Total;
		$this->data['data_raised']= $eth + $btc;
		...
		$result12 =$this->Common_model->select("user_address",$userId); 
		if($result12){
...
public function update_record(){
	...
	$transc = file_get_contents('http://api.etherscan.io/api?module=account&action=txlist&address='.$_POST['ethaddress'].'&sort=desc');
		...
		if(count($data->result)>=$i){
			...
			if(@$data1->isError == '0'){
				...
				if(empty($result)){
					...
					$today =  date('Y-m-d');
					if($today >= '2018-07-13' && $today <='2018-08-23'){
					...
					}else if($today >= '2018-08-24' && $today <='2018-10-12'){
...
public function token_bought(){
	...
	$today =  date('Y-m-d');
	if($today <='2018-08-23'){
		$total =(($data[0]->Total)/3000000)*100;
		print_r($total);
	}else if($today >= '2018-08-24'){
		$total =(($data[0]->Total)/10000000)*100;
		print_r($total);
	}
...
public function enable(){
    ...
    public function enablelogin(){
    	...
    	if($type == 'check'){
    		if($userId!==''){
    			$query = $this->Common_model->select('2_veri',$userId);
           ...
           // print_r($secret);
            ...
            if(!$checkResult)
            {
                ...
                echo '2'; // index function load login page view
...
public function raised(){
	...
	if(($this->data['data_raised'])<=1500000 || ($this->data['data_raised']>=7000000)){

		print_r($this->data['data_raised']);
	}else{
		print_r('Hard Cap Raised');
	}
    	//}
...
public function get_eth_balance(){
	date_default_timezone_get();
	$today =  date('Y-m-d');
	if($today >= '2018-07-14' && $today <='2018-08-24'){
	...
	public function buy_token(){
		...
		if($this->ion_auth->logged_in()){
			$today =  date('Y-m-d');
			if($today >= '2018-06-13' && $today <='2018-08-23'){
				$reduce_supply = get_data_by_id('total_supply','title','PRE-ITS','supply')-($this->input->post('token'));
				$update_query = $this->db->where("title",'PRE-ITS'); $this->db->update("total_supply",array('supply' => $reduce_supply));
			}else if($today >= '2018-08-24' && $today <='2018-10-12'){
				$reduce_supply = get_data_by_id('total_supply','title','ITS','supply')-($this->input->post('token'));
				$update_query = $this->db->where("title",'ITS'); $this->db->update("total_supply",array('supply' => $reduce_supply));
			}
